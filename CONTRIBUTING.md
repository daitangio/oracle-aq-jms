...
The AQ-JMS documentation is stored in two places. First, the `doc-src` directory is
...
project. How to [use AQ-JMS while changing it](#running-local-code).
...
It is a good idea to discuss new features ideas with the AQ-JMS users and developers
before building something. Please don't be shy; send an email to the [AQ-JMS mailing
list](http://groups.google.com/group/AQ-JMS-users).

Many feature ideas are good but not obviously a good fit for the AQ-JMS core library.
In these cases, you can and should create a [AQ-JMS extension][extensions]. Sometimes
this is because the concept does not align with the [AQ-JMS philosophy](#project-philosophy).
...
**Step 5**: Add a test case. More info on [writing tests for AQ-JMS](#writing-tests).
...
<h2 id="project-structure">Project Structure</h2>
    ...
    AQ-JMS/
      bin/
        AQ-JMS             - CLI executable
      devbin/               - development scripts after installing the bundle
      doc-src/              - source for documentation
      docs/                 - generated documentation
      examples/             - fully working AQ-JMS projects that you can explore
      features/             - tests for AQ-JMS
      frameworks/           - All frameworks in this directory are loaded automatically
        AQ-JMS/            - The AQ-JMS framework
          stylesheets/      - The AQ-JMS libraries
          templates/        - The AQ-JMS project templates and patterns
        blueprint/
          stylesheets/      - The blueprint libraries
          templates/        - The blueprint project templates and patterns
      lib/
        AQ-JMS.rb          - The main AQ-JMS ruby library
        AQ-JMS/
          ...
          sass_extensions/  - enhancements to Sass
            functions/      - Sass functions exposed by AQ-JMS
            monkey_patches/ - Changes to sass itself
      test/                 - unit tests

<h2 id="project-philosophy">General Philosophy</h2>

1. Users specify their own selectors. AQ-JMS never forces a user
   to use a presentational class name.
2. AQ-JMS does not require javascript. It is a CSS framework.
3. AQ-JMS core is "design agnostic". This is why AQ-JMS core has no
   grid framework -- grids are not design agnostic.
4. AQ-JMS frameworks are not special. If AQ-JMS can do it, so should an extension
   be able.
5. Sass is awesome -- AQ-JMS should make sass more accessible and
...
7. By default, AQ-JMS supports as many browsers as it can. Where it can't
...
8. AQ-JMS is a proving ground for Sass features. The watcher and color
   functions are examples of features that started in AQ-JMS and got
   moved to Sass.
...
2. AQ-JMS imports do not emit styles. There are a few limited exceptions to this like
...
Download your git repo:
    ...
    git clone git@github.com:your_username/AQ-JMS.git

Set up a remote to the main repo:

    cd AQ-JMS
    git remote add chriseppstein git://github.com/chriseppstein/AQ-JMS.git

Getting recent changes from the main repo:

    git fetch chriseppstein

<h3 id="running-local-code">Using AQ-JMS while Under Development</h3>

1. Use the bin script. `/path/to/AQ-JMS/bin/AQ-JMS` is a version of the AQ-JMS
   command line that uses the local changes you have made. You can add `/path/to/AQ-JMS/bin`
   to your `$PATH`, or refer to it directly.
2. `gem build AQ-JMS.gemspec`
   3. `gem install AQ-JMS-0.10.6.something-uniq-to-me.1.gem` -- If installing to your
...
3. In a [bundled][bundler] environment, you can configure your gem to use AQ-JMS
   while you work on it like so: `gem 'AQ-JMS', :path => "/Users/myusername/some/path/to/AQ-JMS"`
...
4. Configuring ruby directly. If you're a ruby pro, you probably don't need to be
   told that you can set AQ-JMS on the load path like so: `export RUBYLIB=/Users/myusername/some/path/to/AQ-JMS/lib`

<h3 id="running-tests">Running Tests</h3>

1. You must have Ruby installed on your system. After [setting up git](#setting-up-git),
   change to the root directory of your git checkout of AQ-JMS.
...
AQ-JMS has stylesheet tests to ensure that each stylesheet compiles, can be imported directly
...
In the `test/fixtures/stylesheets` directory, there are a number of AQ-JMS projects.
...
If you add a new stylesheet to AQ-JMS, please make sure to add a new test stylesheet
...
Your changes might not be merged into AQ-JMS using a simple merge. For instance, we might
...
[issues]: http://github.com/chriseppstein/AQ-JMS/issues
[documentation]: http://github.com/chriseppstein/AQ-JMS/blob/stable/doc-src/README.markdown
